...
1. The Ghost Mode: Change the background to dark blue and the particles to white with a lower opacity (0.02).

2. Turbulence: In the angle calculation, multiply the result by 20. What happens to the flow?

3. The Vortex: Can you modify the angle formula so all particles spiral toward the center of the screen? (Hint: Use Math.atan2(centerY - p.y, centerX - p.x) + Math.PI/2).

4. Art: Increase the ctx.lineWidth in the advanced_flow.html.
